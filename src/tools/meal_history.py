"""
Meal history utility moved out of `seasonal_finder`.

This module provides a simple `MealHistory` helper used by tests and tools.
The implementation is intentionally minimal and safe to import without optional
dependencies.
"""
...
class MealHistory:
    ...
    def meals_in_range(self, start_date: date, end_date: date) -> List[Dict]:
        return [
            meal for meal in self.meal_data if start_date <= meal["date"] <= end_date
        ]
